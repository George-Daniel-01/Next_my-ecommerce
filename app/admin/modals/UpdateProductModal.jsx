...
import { toggleUpdateProductModal } from "@/app/store/slices/extraSlice";
import { LoaderCircle } from "lucide-react";
import { updateProduct } from "@/app/store/slices/adminProductSlice";
...
const UpdateProductModal = ({ selectedProduct }) => {
  ...
  useEffect(() => {
    if (selectedProduct) setFormData({ name: selectedProduct.name || "", description: selectedProduct.description || "", price: selectedProduct.price || "", category: selectedProduct.category || "", stock: selectedProduct.stock || "" });
  }, [selectedProduct]);

  const handleSubmit = (e) => { e.preventDefault(); dispatch(updateProduct(formData, selectedProduct.id)); };
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-xl w-full max-w-2xl p-6 relative">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-2 px-6 rounded col-span-2">
