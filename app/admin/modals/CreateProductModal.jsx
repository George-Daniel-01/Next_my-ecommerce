...
import { createNewProduct } from "@/app/store/slices/adminProductSlice";
import { toggleCreateProductModal } from "@/app/store/slices/extraSlice";
...
const CreateProductModal = () => {
  ...
  const [formData, setFormData] = useState({ name: "", description: "", price: "", category: "Electronics", stock: "", images: [] });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-xl w-full max-w-2xl p-6 relative">
        <button onClick={() => dispatch(toggleCreateProductModal())} className="absolute top-4 right-4 text-gray-600 hover:text-red-500 text-xl">&times;</button>
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit}>
          ...
          <input type="file" multiple accept="image/*" onChange={(e) => setFormData({...formData, images: Array.from(e.target.files)})} className="border px-4 py-2 rounded col-span-2" />
          <textarea placeholder="Description" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="border px-4 py-2 rounded col-span-2" rows={4} />
          <button type="submit" className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-2 px-6 rounded col-span-2">
            {loading ? <><LoaderCircle className="w-6 h-6 animate-spin" />Creating</> : "Add New Product"}
          </button>
        </form>
      </div>
    </div>
